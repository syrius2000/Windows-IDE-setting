# ソフトウェア構成表（Software Bill of Materials）

本プラットフォームで利用・導入するツールの構成定義、ライセンス、配布元、固定バージョン、および運用手順です。

> 注: ここでの「BOM」は **Bill of Materials（構成表）** の意味です。UTF-8 の Byte Order Mark ではありません。
...
| **Python** | RWD処理・AI連携・検証 | PSF License | [python.org](https://www.python.org/) | Win/Mac | `uv python install 3.12.14` | **3.12.14** (固定) | セキュリティリリース時に `uv python install <ver>` で計画更新 | `uv python uninstall 3.12.14` | **必須** | パッチまで固定。`3.12` 未指定は不可。仮想環境（.venv）で完全分離 |
...
| **rig** | R言語バージョンマネージャー | MIT | [github.com/r-lib/rig](https://github.com/r-lib/rig) | Win/Mac | `winget install --id Posit.rig --exact` / `brew install --cask rig` | 0.7+ | `rig self update` | `winget uninstall --id Posit.rig --exact` | **必須** | WindowsはR本体をCRANから導入し、rigはRtools管理に使用（`02-install-analysis.ps1`）。WinGet ID は `Posit.rig`（`RProject.rig` は存在しない） |
| **R** | 統計解析・生存時間解析 | GPL v2 / GPL v3 | [r-project.org](https://www.r-project.org/) | Win/Mac | Win: 公式CRANインストーラー（R-4.6.1-win.exe） / Mac: `rig add 4.6.1` | 4.6.1 | Win: CRAN最新版を確認 / Mac: `rig add <ver>` | Win: Windowsのアプリ設定 / Mac: `rig rm 4.6.1` | **必須** | Windowsは `scripts/windows/02-install-analysis.ps1` が公式CRAN URLから取得。パッケージは `renv` で管理 |
...
| **TypeScript** | 型付き報告書・PPTX生成 | Apache-2.0 | [typescriptlang.org](https://www.typescriptlang.org/) | Win/Mac | `pnpm add -g typescript ts-node`（`03-install-reporting.ps1`） | 5.6+ | `pnpm update -g typescript` | `pnpm remove -g typescript` | **必須** | Case Project は `pnpm run report:pptx`（`src/typescript/`）で編集可能PPTXを生成 |
...
| **PptxGenJS** | 編集可能PowerPoint出力 | MIT | [gitbrent.github.io/PptxGenJS](https://gitbrent.github.io/PptxGenJS/) | Win/Mac | `pnpm add pptxgenjs` (Project内) | 3.12+ | `pnpm update pptxgenjs` | - | **必須** | 成果物PowerPointをプログラム生成。`pnpm report:pptx` |
...
| **Gitleaks** | 秘密情報・APIキー誤混入検知 | MIT | [github.com/gitleaks/gitleaks](https://github.com/gitleaks/gitleaks) | Win/Mac | `winget install Gitleaks.Gitleaks` / `brew install gitleaks` | 8.21+ | `winget upgrade Gitleaks.Gitleaks` | `winget uninstall Gitleaks.Gitleaks` | **推奨（任意）** | `01-install-common.ps1` で導入。`04-configure.ps1` が pre-commit フックを設定 |
| **SAS Foundation 9.4** | 既存SAS資産・マクロ実行 | プロプライエタリ | [sas.com](https://www.sas.com/) | Win | 手動インストール（ライセンス必要） | 9.4 | ベンダー手順 | ベンダー手順 | **任意** | セットアップ対象外。未導入でも Python/R 解析は可能 |
